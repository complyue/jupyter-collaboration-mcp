...
async def test_list_notebooks(notebook_handlers, mock_rtc_adapter):
    ...
    mock_rtc_adapter.list_notebooks = AsyncMock(
        return_value=[
            {
                "path": "/test.ipynb",
                "name": "Test Notebook",
    ...
    # Call the handler
    result = await handler(name="list_notebooks", arguments={"path": "/test"})
...
async def test_get_notebook(notebook_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.get_notebook = AsyncMock(
        return_value={
            "path": "/test.ipynb",
            "content": {"cells": []},
            "format": "json",
    ...
    result = await handler(
        name="get_notebook", arguments={"path": "/test.ipynb", "include_collaboration_state": True}
    )
    ...
    # Check that the RTC adapter was called correctly
    mock_rtc_adapter.get_notebook.assert_called_once_with("/test.ipynb", True)
...
async def test_get_notebook_missing_path(notebook_handlers):
    ...
    with pytest.raises(ValueError, match="Path is required"):
        await handler(name="get_notebook", arguments={})
...
async def test_create_notebook_session(notebook_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.create_notebook_session = AsyncMock(
        return_value={
    ...
    # Call the handler
    result = await handler(name="create_notebook_session", arguments={"path": "/test.ipynb"})
...
async def test_update_notebook_cell(notebook_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.update_notebook_cell = AsyncMock(
        return_value={"success": True, "cell_id": "test-cell-id", "timestamp": 1234567890}
    )
...
async def test_list_documents(document_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.list_documents = AsyncMock(
        return_value=[
            {
                "path": "/test.md",
                "name": "Test Document",
    ...
    # Call the handler
    result = await handler(
        name="list_documents", arguments={"path": "/test", "file_type": "markdown"}
    )
...
async def test_get_document(document_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.get_document = AsyncMock(
        return_value={
            "path": "/test.md",
    ...
    # Call the handler
    result = await handler(
        name="get_document", arguments={"path": "/test.md", "include_collaboration_state": True}
    )
    ...
    # Check that the RTC adapter was called correctly
    mock_rtc_adapter.get_document.assert_called_once_with("/test.md", True)
...
async def test_update_document(document_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.update_document = AsyncMock(
        return_value={"success": True, "path": "/test.md", "version": 2, "timestamp": 1234567890}
    )
...
async def test_fork_document(document_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.fork_document = AsyncMock(
        return_value={
            "success": True,
...
async def test_get_online_users(awareness_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.get_online_users = AsyncMock(
        return_value=[
            {
                "id": "user1",
                "name": "User 1",
    ...
    # Call the handler
    result = await handler(name="get_online_users", arguments={"document_path": "/test.ipynb"})
...
async def test_get_user_presence(awareness_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.get_user_presence = AsyncMock(
        return_value={
            "user_id": "user1",
            "status": "online",
            "last_activity": 1234567890,
    ...
    # Call the handler
    result = await handler(
        name="get_user_presence", arguments={"user_id": "user1", "document_path": "/test.ipynb"}
    )
    ...
    # Check that the RTC adapter was called correctly
    mock_rtc_adapter.get_user_presence.assert_called_once_with("user1", "/test.ipynb")
...
async def test_get_user_presence_missing_user_id(awareness_handlers):
    ...
    with pytest.raises(ValueError, match="User ID is required"):
        await handler(name="get_user_presence", arguments={})
...
async def test_update_cursor_position(awareness_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.update_cursor_position = AsyncMock(
        return_value={
            "success": True,
            "user_id": "user1",
    ...
    mock_rtc_adapter.update_cursor_position.assert_called_once_with(
        "/test.md",
        {"line": 5, "column": 10},
        {"start": {"line": 5, "column": 5}, "end": {"line": 5, "column": 15}},
    )
...
async def test_join_session(awareness_handlers, mock_rtc_adapter):
    ...
    # Mock the RTC adapter response
    mock_rtc_adapter.join_session = AsyncMock(
        return_value={"success": True, "session_id": "test-session-id", "timestamp": 1234567890}
    )
    ...
    # Call the handler
    result = await handler(name="join_session", arguments={"session_id": "test-session-id"})
...
async def test_unknown_tool(notebook_handlers):
    ...
    with pytest.raises(ValueError, match="Unknown tool"):
        await handler(name="unknown_tool", arguments={})
