...
from jupyter_collaboration_mcp.auth import (
    AuthConfig, AuthManager, ResourceAuthorizer, authenticate_mcp_request,
    AuthorizedMCPHandler, get_auth_manager, get_authorizer, configure_auth,
    get_secret_key, create_auth_token
)
...
async def test_check_document_access_allowed(resource_authorizer):
    ...
    result = await resource_authorizer.check_document_access(
        user_claims, "/test.md", "read"
    )
...
async def test_check_session_access_allowed(resource_authorizer):
    ...
    result = await resource_authorizer.check_session_access(
        user_claims, "test-session", "join"
    )
...
async def test_check_session_access_denied(resource_authorizer):
    ...
    result = await resource_authorizer.check_session_access(
        user_claims, "test-session", "manage"
    )
...
async def test_authenticate_mcp_request_invalid_header():
    ...
    scope = {
        "headers": [
            (b"authorization", b"InvalidHeader test-token")
        ]
    }
